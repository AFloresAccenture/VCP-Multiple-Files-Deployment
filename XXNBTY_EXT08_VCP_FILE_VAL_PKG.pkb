...
13-May-2015					Erwin Ramos				Added execution of multiple files.
15-May-2015					Erwin Ramos				Added sleep command in the exec_concurrent_main procedure. 
*/
--------------------------------------------------------------------------------------------
...
IS
  ...
  PROCEDURE exec_concurrent_main (x_errbuf              	OUT VARCHAR2
								 ...
								 ,p_archive					VARCHAR2	--5/7/2015 ERamos
							 )
  IS 
  ...
  BEGIN  
	v_step := 1;
	v_cntr := 0;
	v_sleepcnt := p_interval * 60;  -- 15-May-2015: ERAMOS: Added for sleep command.
	--Cursor to call EBS DBLINK
	 ...
	 v_step := 3;
	    ...
	    -- 15-May-2015: ERAMOS: Added if the status is not C it will wait depending on the interval.
			LOOP
				v_cntr := v_cntr + 1;
				v_step := 4;		
				OPEN c_ebs_req_load FOR query_str;
				FETCH c_ebs_req_load INTO v_executed;
				...
				ELSIF
					v_cntr <= p_retries 
				THEN
					dbms_lock.sleep( v_sleepcnt );
				ELSIF  v_executed <> 'C' AND v_cntr > p_retries
				THEN
					-- If the EBS INT REQUEST SET load did not execute for today. 
				...
				END IF;
			END LOOP;			
			-- 15-May-2015: ERAMOS: END
			
		--The EBS request set is successfully completed. 
        --To call the XXNBTY_VCP_LEG_FILE_VAL concurrent program
			v_step := 7;
			...
			ln_wait := fnd_concurrent.wait_for_request( request_id      => v_request_id
						...
						ELSE 
							...
							x_retcode := 0;
						END IF;
							FND_FILE.PUT_LINE(FND_FILE.LOG,'The XXNBTY_VCP_MULTIPLE_FILE_CHECK will execute to check the multiple files. ');
							---13-May-2015: Added this line to execute the multiple files validation. 
							v_step := 12;
							v_request_id := FND_REQUEST.SUBMIT_REQUEST(application  => 'XXNBTY'
																		,program      => 'XXNBTY_VCP_MULTIPLE_FILE_CHECK'
																		,start_time   => TO_CHAR(SYSDATE,'DD-MON-YYYY HH:MI:SS')
																		,sub_request  => FALSE
